<template>
  <div class="grid-comp">
    <table class="myTable" v-if="myTable">
      <caption class="mainTitle">
        Example Row Organized Stach Table
      </caption>
      <tr v-for="(row, rowIndex) in myTable.data.rows" :key="rowIndex">
        <template v-if="isHeader(row)">
          <template v-for="(cellValue, colIndex) in row.cells">
            <th
              v-if="!isHidden(myTable, row, colIndex)"
              :key="colIndex"
            ...
            >
              <div
                class="title"
                :style="{ 'padding-left': leftPadding(row, colIndex) }"
              >
                <span>{{ cellValue }}</span>
                <span
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const toggleSortDirection = (row: IRow, colIndex: number) => {
      const headerCellDetail = row.headerCellDetails?.[
        colIndex
      ] as IMyHeaderCellDetail;
    ...
    const onSortArrowClicked = (row: IRow, colIndex: number) => {
      ...
      if (myTable.value?.data?.rows) {
        myTable.value.data.rows = sorted(
          myTable.value.data.rows,
          colIndex,
          isOrderedAsc.value
        );
      }
